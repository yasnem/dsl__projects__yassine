 ...
 *  Filename:
 *
 *     $Id: dgrp_specproc.c,v 1.6 2014/03/28 17:21:28 pberger Exp $
 *
 *  Description:
 *
...
static ssize_t dgrp_gen_proc_read(struct file *, char *, size_t, loff_t *);
static ssize_t dgrp_gen_proc_write(struct file *, const char *, size_t, loff_t *);
static int parse_write_config(char *);
#if LINUX_VERSION_CODE < KERNEL_VERSION(3,10,0)
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,38) && LINUX_VERSION_CODE < KERNEL_VERSION(3,1,0)
static int proc_chk_perm(struct inode *, int, unsigned int);
#else
...
static void register_proc_table(struct dgrp_proc_entry *, struct proc_dir_entry *);
static void unregister_proc_table(struct dgrp_proc_entry *, struct proc_dir_entry *);
...
static struct dgrp_proc_entry dgrp_table[] = {
	{DGRP_CONFIG,   "config", 0644, NULL, &read_config, &write_config,
			NULL, __SEMAPHORE_INITIALIZER(dgrp_table[0].excl_sem, 1), 0},
	{DGRP_INFO,     "info", 0644, NULL, &read_info, &write_info,
			NULL, __SEMAPHORE_INITIALIZER(dgrp_table[1].excl_sem, 1), 0},
	{DGRP_NODEINFO, "nodeinfo", 0644, NULL, &read_nodeinfo, NULL,
			NULL, __SEMAPHORE_INITIALIZER(dgrp_table[2].excl_sem, 1), 0},
...
static int test_perm(int mode, int op)
{
	...
	if (!current_euid().val)
		mode >>= 6;
	else if (dgrp_in_egroup_p((kgid_t){0}))
		mode >>= 3;
#endif
	if ((mode & op & 0007) == op)
		return 0;
				...
				struct proc_dir_entry *root)
{
	...
	for (; table->magic; table++) {
		...
		if (!de) {
			de = dgrp_create_proc_entry(table->name, mode, root);
			if (!de)
				continue;
			de->data = (void *) table;
			if (!table->child) {
...
#else
				unregister_net_device(tmp, net_entry_pointer);
#endif
...
#else
				unregister_mon_device(tmp, mon_entry_pointer);
#endif
...
#else
				unregister_dpa_device(tmp, dpa_entry_pointer);
#endif
...
#else
				unregister_ports_device(tmp, ports_entry_pointer);
#endif
				...
				struct proc_dir_entry *root)
{
	...
	for (; table->magic; table++) {
		...
		mode = table->mode;
		if (!table->child) {
			de = proc_create_data(table->name, mode, root,
					&dgrp_proc_file_ops, table);
			table->de = de;
		} else {
			de = proc_mkdir_data(table->name, mode, root, table);
			table->de = de;
			register_proc_table(table->child, de);
		}
...
static ssize_t dgrp_gen_proc_read(struct file *file, char *buf, size_t count, loff_t *ppos)
{
	struct dgrp_proc_entry *entry;
...
static ssize_t dgrp_gen_proc_write(struct file *file, const char *buf, size_t count, loff_t *ppos)
{
	struct dgrp_proc_entry *entry;
	dgrp_proc_handler *handler;
	ssize_t res;
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(3,10,0)
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,38) && LINUX_VERSION_CODE < KERNEL_VERSION(3,1,0)
static int proc_chk_perm(struct inode *inode, int op, unsigned int flags)
#else
 ...
 */
static int write_config(struct dgrp_proc_entry *table, int dir,
			struct file *filp, void *buffer,
			ssize_t *lenp, loff_t *ppos)
{
	int  retval;
	long len;
#define INBUFLEN 256
...
static int read_config(struct dgrp_proc_entry *table, int dir,
			struct file *filp, void *buffer,
			ssize_t *lenp, loff_t *ppos)
{
...
static int parse_add_config(char *buf)
{
	 ...
	 */
	new_nd = dgrp_kzmalloc(sizeof(struct nd_struct), GFP_KERNEL);
	if (!new_nd) {
		dbg_trace(("kmalloc failed: struct nd_struct (%d)\n",
...
static int parse_del_config(char *buf)
{
	...
	if (nd->nd_dpa_de)
		(void) unregister_dpa_device(nd, dpa_entry_pointer);
...
static int write_info(struct dgrp_proc_entry *table, int dir,
			struct file *filp, void *buffer,
			ssize_t *lenp, loff_t *ppos)
{
	static int state = 0;
...
static int read_info(struct dgrp_proc_entry *table, int dir,
			struct file *filp, void *buffer,
			ssize_t *lenp, loff_t *ppos)
{
	static int currline = 0;
	...
	if (!linepos) {
		...
		case INFO_CHAR:
			sprintf(linebuf, "%s:%*s0x%02x\t(%d)\n",
				info_vars[currline].name, padlen, "",
...
static int read_nodeinfo(struct dgrp_proc_entry *table, int dir,
			struct file *filp, void *buffer,
			ssize_t *lenp, loff_t *ppos)
{
	static struct nd_struct *curr_ps = NULL;
	...
	if (!linepos) {
		...
		case 1:
			...
			break;
		case 2:
			strcpy(linebuf, "# ID  State       Version  ID   Version  Description\n");
			break;
		default:
			ID_TO_CHAR(curr_ps->nd_ID, tmp_id);
			...
			if (curr_ps->nd_state == NS_READY) {
				...
				sprintf(linebuf,
					...
					tmp_id,
					ND_STATE_STR(curr_ps->nd_state),
					hwver, curr_ps->nd_hw_id, swver, curr_ps->nd_ps_desc);
			} else {
				sprintf(linebuf, "  %-2.2s  %-10.10s\n",
					tmp_id,
					ND_STATE_STR(curr_ps->nd_state));
			}
			curr_ps = curr_ps->nd_inext;
