 ...
 *  Filename:
 *
 *     $Id: dgrp_tty.c,v 1.6 2014/03/28 17:21:28 pberger Exp $
 *
 *  Description:
 *
...
static int drp_wait_ack(struct ch_struct *, unsigned long *, int ttylock);
static void dgrp_tty_close(struct tty_struct *, struct file *);

/* ioctl helper functions */
static int dgrp_print_KME(struct ch_struct *);
...
static int send_break(struct ch_struct *, int);
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(3,0,0)
static int dgrp_tty_tiocmget(struct tty_struct *, struct file *);
static int dgrp_tty_tiocmset(struct tty_struct *, struct file *, unsigned int, unsigned int);
#else
...
static void drp_param(struct ch_struct *ch)
{
	...
	if (ch->ch_tun.un_open_count) {
		...
		dbg_tty_trace(IOCTL, ("drp_param(%x): start\n",
			MINOR(tty_devnum(ch->ch_pun.un_tty))));
...
*    int ttylock      whether need to step around gbl.tty lock
*
* Return Values:
*
*    0        Success
...
static int drp_wait_ack(struct ch_struct *ch, ulong *lock_flags, int ttylock)
{
	struct nd_struct *nd;
	int in;
	int i;
	...
	for (i = 0; i < n; i++) {
		...
		for (;;) {
			 ...
			 */
/* Note: I think this is a dead codepath since I don't see
 * any code that sets GLBL(wait_control) */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,0,0)
#if LINUX_VERSION_CODE < KERNEL_VERSION(3,7,0)
if (ttylock) tty_unlock();
#else
if (ttylock) tty_unlock(ch->port.tty);
#endif
#endif
...
if (ttylock) tty_lock();
#else
if (ttylock) tty_lock(ch->port.tty);
#endif
#endif
	...
	for (;;) {
		 ...
		 */
			...
			if (ch->ch_state == CS_READY && ch->ch_otype == otype)
				...
				dbg_tty_trace(OPEN, ("tty open:(%x) port not CS_READY yet"
					" or otype(%x) not ch_otype(%x)\n",
					MINOR(tty_devnum(tty)), otype,
					ch->ch_otype));
		}
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,0,0)
/* release global tty system lock that was taken above us in 
 * drivers/tty/tty_io.c:tty_open().. around the schedule */
#if LINUX_VERSION_CODE < KERNEL_VERSION(3,7,0)
  tty_unlock();
#else
  tty_unlock(tty);
  ...
  tty_lock(tty);
	...
	if (ch->ch_open_count == 1) {
		...
		if (OPEN_SETS_CLOCAL(category) || C_CLOCAL(tty) ||
		    ch->ch_s_mlast & DM_CD ||
	...
	if (un->un_open_count == 1) {
		...
		if (!(un->un_flag & UN_STICKY)) {
			...
			(un->un_tty)->termios->c_lflag = ECHOK | ECHO |
							ICANON | ISIG | IEXTEN;
		}
#endif

		if (OPEN_SETS_CLOCAL(MINOR(tty_devnum(tty->device))))
#if LINUX_VERSION_CODE < KERNEL_VERSION(3,7,0)
		...
		if (GLBL(wait_control))
			drp_wait_ack(ch, &lock_flags, 1); /* global tty_lock is held in open */
	}
	...
	printk(KERN_ALERT "compat_ioctl: %p\n",
...
unlock:
	...
	dbg_tty_trace(OPEN, ("tty open (%x): done, ch_count(%d) un_count(%d) tty_count(%d)\n",
			MINOR(tty_devnum(tty)), ch->ch_open_count,
			un->un_open_count, tty->count));
...
static void dgrp_tty_close(struct tty_struct *tty, struct file *file)
{
	...
	dbg_tty_trace(CLOSE, ("tty close(%x): current ch_count(%d) un_count(%d)\n",
		MINOR(tty_devnum(tty)), ch->ch_open_count, un->un_open_count));
		    ...
		    ((long)(jiffies - waketime) >= 0 &&
		      (ch->ch_digi.digi_flags & DIGI_PRINTER) == 0)) {
				dbg_tty_trace(CLOSE, ("tty close(%x) Flushing line discipline etc. flag: %x left:%d\n",
					MINOR(tty_devnum(tty)),
					(file->f_flags & (O_NDELAY | O_NONBLOCK)),
	...
	if (GLBL(wait_control) && !err)
		drp_wait_ack(ch, &lock_flags,1); /* Glb.tty_lock is held in close */
...
static int dgrp_calculate_txprint_bounds(struct ch_struct *ch, int space, int *un_flag)
{
	clock_t tt;
	clock_t mt;
	...
	if (ch->ch_tun.un_open_count != 0 && ch->ch_tun.un_tty->ops->chars_in_buffer &&
...
static int dgrp_tty_write(struct tty_struct *tty, const unsigned char *buf, int count)
{
	...
	dbg_tty_trace(WRITE, ("dgrp_tty_write(%x): tty=%lx user=%d count=%d\n",
		   MINOR(tty_devnum(tty)), (long) tty, from_user, count));
...
static int dgrp_tty_put_char(struct tty_struct *tty, unsigned char new_char)
{
	...
	dbg_tty_trace(WRITE, ("dgrp_tty_put_char(%x): tty=%lx char(%x)\n",
		   MINOR(tty_devnum(tty)), (long)tty, new_char));
...
static int dgrp_tty_write_room(struct tty_struct *tty)
{
	...
	dbg_tty_trace(WRITE, ("dgrp_tty_write_room(%x) count(%d) busy(%d)\n",
		      MINOR(tty_devnum(tty)), count, un->un_tbusy));
...
static int dgrp_tty_chars_in_buffer(struct tty_struct *tty)
{
	...
	dbg_tty_trace(WRITE, ("dgrp_tty_chars_in_buffer(%x) ch_buf(%d) +ch_s_tbuf=(%d) busy(%d)\n",
		      MINOR(tty_devnum(tty)), count1, count, un->un_tbusy));
 ...
 */
static int send_break(struct ch_struct *ch, int msec)
{
	ulong x;
 ...
 */
#if LINUX_VERSION_CODE < KERNEL_VERSION(3,0,0)
static int dgrp_tty_tiocmset(struct tty_struct *tty, struct file *file,
...
#endif
{
 ...
 m = ch->ch_mout;
	...
	if (set & TIOCM_RTS)   m |= DM_RTS;

	if (set & TIOCM_DTR)   m |= DM_DTR;

	if (clear & TIOCM_RTS) m &= ~(DM_RTS);

	if (clear & TIOCM_DTR) m &= ~(DM_DTR);
 ...
 */
static int dgrp_tty_digiseta(struct tty_struct *tty, struct digi_struct *new_info)
{
	struct un_struct *un = tty->driver_data;
	struct ch_struct *ch;
	...
	dbg_tty_trace(IOCTL, ("dgrp_tty_digiseta(%x) - done, flags set(%x)\n",
			MINOR(tty_devnum(tty)), ch->ch_digi.digi_flags));
...
#else
dgrp_tty_ioctl(struct tty_struct *tty, unsigned int cmd, unsigned long arg)
#endif
{
	struct un_struct *un;
	struct ch_struct *ch;
	...
	case TIOCSSERIAL:
	case TIOCSERCONFIG:
   return -ENOIOCTLCMD; /* dontcare.  n/a for RealPort */
 break;

	case TIOCMIWAIT:
	{
 	 uchar m,c;
		/* arg is a value mask of TIOCM_[RNG|DSR|CD|CTS] and the idea is that
		   if eg. RNG is set, then we wait for our RNG state to change.  I.e.
		   we are not waiting for RNG to become set, just to change
     ..i dont _think_ there is need to LOCK around this...
		 */
	 m = ((ch->ch_s_mlast & ~(DM_RTS|DM_DTR)) | (ch->ch_mout & (DM_RTS|DM_DTR)));
		for (;;) {
		 /* wait for something to change : NB: tty_lock not held on IOCTL..
		  * The monolithic and 'invariant' code in dgrp_net_ops does not issue
		  * any wakeups on modem changes unless we're waiting for carrier, so
		  * instead of changing that code, poll.. */
			msleep (100); /* sleep 1/10 of a second. */
			if (signal_pending(current))  return -ERESTARTSYS;
			if (un->un_flag & UN_CLOSING) return -EIO;
			c = ((ch->ch_s_mlast & ~(DM_RTS|DM_DTR)) | (ch->ch_mout & (DM_RTS|DM_DTR)));
			if ((c ^= m)) {
			if (((arg & TIOCM_RNG) && (c & DM_RI))  ||
			    ((arg & TIOCM_DSR) && (c & DM_DSR)) ||
			    ((arg & TIOCM_CD)  && (c & DM_CD))  ||
			    ((arg & TIOCM_CTS) && (c & DM_CTS)))
				return 0;
		 }
	 }
	}
 break;
	...
	{
	 unsigned char lsr;
		/* well, its FIFO information, so intepret as txempty? */
		lsr = (ch->ch_tout == ch->ch_tin);
		return put_user(lsr, (unsigned int *)arg);
		break;
	}
#endif
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,31)
		if (tty->ldisc->ops->ioctl && !IS_PRINT(MINOR(tty_devnum(tty)))) {
#else
		if (tty->ldisc.ops->ioctl && !IS_PRINT(MINOR(tty_devnum(tty)))) {
#endif
			int retval;
...
#endif
#else
   retval = -ENOIOCTLCMD; /* so that caller proceeds to the ldisc version */
#endif
#if LINUX_VERSION_CODE < KERNEL_VERSION(3,7,0)
	...
	case TCSETSW:
		 ...
		 */
		if (!IS_PRINT(MINOR(tty_devnum(tty))) || !ch->ch_tun.un_open_count) {
			rc = tty_check_change(tty);
			if (rc)
				return rc;
		}
...
static void dgrp_tty_set_termios(struct tty_struct *tty, struct ktermios *old)
{
	...
	dbg_tty_trace(INPUT, ("dgrp_tty_set_termios(%x) cflag(%x) oflag(%x) lflag(%x) iflag(%x)\n",
		      MINOR(tty_devnum(tty)), ts->c_cflag, ts->c_oflag,
		      ts->c_lflag, ts->c_iflag));
...
static void dgrp_tty_hangup(struct tty_struct *tty)
{
	...
	dbg_tty_trace(INPUT, ("dgrp_tty_hangup(%x) c_hupcl(%x)\n",
		MINOR(tty_devnum(tty)), C_HUPCL(tty)));
...
dgrp_tty_init(struct nd_struct *nd)
{
	...
	nd->nd_serial_ttdriver->flags        = (TTY_DRIVER_REAL_RAW | TTY_DRIVER_DYNAMIC_DEV | TTY_DRIVER_HARDWARE_BREAK);

	/* The kernel wants space to store pointers to tty_structs. */
	nd->nd_serial_ttdriver->ttys =
		dgrp_kzmalloc(CHAN_MAX * sizeof(struct tty_struct *), GFP_KERNEL);
	if (!nd->nd_serial_ttdriver->ttys)
		return -ENOMEM;
	...
	nd->nd_callout_ttdriver->driver_name  = DRVSTR "(dgrp)";
	nd->nd_callout_ttdriver->flags        = (TTY_DRIVER_REAL_RAW | TTY_DRIVER_DYNAMIC_DEV | TTY_DRIVER_HARDWARE_BREAK);
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,28)
	nd->nd_callout_ttdriver->refcount = nd->nd_tty_ref_cnt;
#endif
	...
	nd->nd_xprint_ttdriver->driver_name   = DRVSTR "(dgrp)";
	nd->nd_xprint_ttdriver->flags         = (TTY_DRIVER_REAL_RAW | TTY_DRIVER_DYNAMIC_DEV | TTY_DRIVER_HARDWARE_BREAK);
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,28)
	nd->nd_xprint_ttdriver->refcount = nd->nd_tty_ref_cnt;
#endif
