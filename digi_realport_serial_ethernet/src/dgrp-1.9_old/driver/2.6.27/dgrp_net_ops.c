 ...
 *  Filename:
 *
 *     $Id: dgrp_net_ops.c,v 1.12 2014/04/17 04:33:26 pberger Exp $
 *
 *  Description:
 *
 ...
 * n_tty.c's private struct n_tty_data.  So... here we mimic
 * what n_tty_set_termios() does to set/clear "real_raw".
 */
...
static void dgrp_input(struct ch_struct *ch)
{
...
#else
	dbg_net_trace(INPUT, ("rawreadok(%d) data_len(%d) len(%d)\n",
		GLBL(rawreadok), data_len, len));
#endif
	...
	if (len && !(ch->ch_flag & CH_RXSTOP)) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,8,0)
		unsigned char l_real_raw = dgrp_is_real_raw(tty);
#endif
		...
		if (GLBL(rawreadok) && l_real_raw)
#endif
			dbg_net_trace(INPUT, ("Have rawreadok and tty->real_raw!\n"));
...
#endif
		{
			...
			tty_insert_flip_string_flags(&ch->port, myflipbuf, myflipflagbuf, len);
