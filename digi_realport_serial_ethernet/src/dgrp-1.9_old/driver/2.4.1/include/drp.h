...
#define IDENT_DRP_H "$Id: drp.h,v 1.2 2010/02/04 20:06:09 scottk Exp $"
...
#define DPA_HIGH_WATER	58000		/* Enforce flow control when over this amount */
 ...
 *
 *  ALWAYS_FORCES_CARRIER(cat)	Carrier is always forced high.
 *
 *  OPEN_SETS_CLOCAL(cat)	CLOCAL is set on open.
 *
 *  HARD_HANGUP(cat)		The device is rendered inoperative after
 *				carrier loss.  It must be closed and opened
 *				again before it can be used (ala STREAMS).
 */
...
#define ALWAYS_FORCES_CARRIER(cat)	0
#define OPEN_SETS_CLOCAL(cat)		0
#define HARD_HANGUP(cat)		0
 ...
 */
#if 0
extern lock_t drp_poll_lock;		/* Global lock */

extern b_sema_t drp_allocate_semaphore;	/* Semaphore to serialize net opens */
#endif
 ...
 *
 */
#define UN_HUP_NOTIFY	0x0001 /* Notify getty on hangups and closes
                                   on the callout port */
...
struct un_struct
{
    ...
    ushort     un_blocked_open;		/* Reference count */
    int		un_flag;		/* Unit flags */
    ushort     un_tbusy;		/* Busy transmit count */

    WAITQ_HEAD  un_open_wait;		/* Do I really need to explain? */
    WAITQ_HEAD  un_close_wait;		/* See above explanation */
    ushort	un_type;
#if defined(DGRP_SYSFS_SUPPORT)
    struct device *un_sysfs;		/* Hang our sysfs stuff off of here */
#endif
};
...
#define CS_IDLE		0		/* Channel is idle */
#define CS_WAIT_OPEN	1		/* Waiting for Immediate Open Resp */
#define CS_WAIT_CANCEL	2		/* Waiting for Per/Incom Cancel Resp */
#define CS_WAIT_FAIL	3		/* Waiting for Immed Open Failure */
#define CS_SEND_QUERY	4		/* Ready to send Port Query */
#define CS_WAIT_QUERY	5		/* Waiting for Port Query Response */
#define CS_READY	6		/* Ready to accept commands and data */
#define CS_SEND_CLOSE	7		/* Ready to send Close Request */
#define CS_WAIT_CLOSE	8		/* Waiting for Close Response */
...
#define CH_WAITING_SYNC 0x10000		/* A pending sync was assigned to this port. */
...
struct ch_struct
{
	...
	uchar*	ch_tbuf;		/* Local Transmit Buffer */
	uchar*	ch_rbuf;		/* Local Receive Buffer */
	ulong	ch_cpstime;		/* Printer CPS time */
	ulong	ch_waketime;		/* Printer wake time */

	ulong	ch_flag;		/* CH_* flags */

	ushort	ch_state;		/* CS_* Protocol state */
	...
	uchar	ch_otype;		/* Open request type */
	uchar	ch_pscan_savechar;	/* Last character read by parity scan */
	uchar	ch_pscan_state;		/* PScan State based on last 2 chars */
	uchar	ch_otype_waiting;	/* Type of open pending in server */
	uchar	ch_flush_seq;		/* Receive flush end sequence */
	uchar	ch_s_mlast;		/* Realport MLAST */

	uchar	ch_mout;		/* Local MOUT */
	uchar	ch_mflow;		/* Local MFLOW */
	uchar	ch_mctrl;		/* Local MCTRL */
	uchar	ch_xon;			/* Local XON */
	uchar	ch_xoff;		/* Local XOFF */
	uchar	ch_lnext;		/* Local LNEXT */
	uchar	ch_xxon;		/* Local XXON */
	uchar	ch_xxoff;		/* Local XXOFF */

	uchar	ch_s_mout;		/* Realport MOUT */
	uchar	ch_s_mflow;		/* Realport MFLOW */
	uchar	ch_s_mctrl;		/* Realport MCTRL */
	uchar	ch_s_xon;		/* Realport XON */
	uchar	ch_s_xoff;		/* Realport XOFF */
	uchar	ch_s_lnext;		/* Realport LNEXT */
	uchar	ch_s_xxon;		/* Realport XXON */
	uchar	ch_s_xxoff;		/* Realport XXOFF */

	WAITQ_HEAD ch_flag_wait;	/* Wait queue for ch_flag changes */
	WAITQ_HEAD ch_sleep;		/* Wait queue for my_sleep() */
...
#define NS_CLOSED	0		/* Network device is closed */
#define NS_IDLE		1		/* Network connection inactive */
#define NS_SEND_QUERY	2		/* Send server query */
#define NS_WAIT_QUERY	3		/* Wait for query response */
#define NS_READY	4		/* Network ready */
#define NS_SEND_ERROR	5		/* Must send error hangup */
...
struct nd_struct
{
	...
	struct nd_struct	*nd_inext;/* Next node in linked chain     */
	struct nd_struct	*nd_iprev;/* Prev node in linked chain     */
	struct nd_struct	*nd_hnext;/* Next node in hash table       */
	...
	SPINLOCK_DECLARE(nd_lock);         /* General node lock             */

	struct semaphore nd_net_semaphore; /* Net read/write lock           */
	struct semaphore nd_mon_semaphore; /* Monitor buffer lock           */
	SPINLOCK_DECLARE(nd_dpa_lock);	   /* DPA buffer lock           */

	struct semaphore nd_ports_semaphore; /* /proc/dgrp/ports exclusivity  */
	int           nd_ports_inuse;      /* use count protected by sema.  */

	int           nd_state;            /* NS_* network state            */
	int           nd_chan_count;       /* # active channels             */
	int           nd_flag;             /* Node flags                    */
	int           nd_send;             /* Responses to send             */
	int           nd_expect;           /* Responses we expect           */

	uchar       *nd_iobuf;            /* Network R/W Buffer            */
	WAITQ_HEAD    nd_tx_waitq;         /* Network select wait queue     */

	uchar         *nd_inputbuf;        /* Input Buffer                  */
	uchar         *nd_inputflagbuf;    /* Input Flags Buffer            */
	...
	link_t        nd_link;             /* Link speed params.            */
	...
	WAITQ_HEAD    nd_mon_wqueue;      /* Monitor wait queue (on flags)  */
	uchar       *nd_mon_buf;         /* Monitor buffer                 */

	ulong        nd_dpa_lbolt;	/* DPA start time             */
	int          nd_dpa_flag;	/* DPA flags                  */
	int          nd_dpa_in;		/* DPA in pointer             */
	int          nd_dpa_out;	/* DPA out pointer            */
	WAITQ_HEAD   nd_dpa_wqueue;	/* DPA wait queue (on flags)  */
	uchar        *nd_dpa_buf;	/* DPA buffer                 */

	uint	     nd_dpa_debug;
	uint	     nd_dpa_port;

	WAITQ_HEAD   nd_seq_wque[SEQ_MAX];   /* TX thread wait queues      */
	uchar        nd_seq_wait[SEQ_MAX];   /* Transmit thread wait count */
	...
	uchar        nd_ps_desc[MAX_DESC_LEN+1];  /* Description from PS   */
	uint	     nd_vpd_len;		/* VPD len, if any */   
	uchar	     nd_vpd[VPDSIZE];		/* VPD, if any */
	...
	struct semaphore nd_writebuf_semaphore; /* Protects access to the writebuf below */
	uchar	     *nd_writebuf;		/* Used to cache data read from user */

	struct ch_struct nd_chan[CHAN_MAX];  /* Channel array                  */

#if defined(DGRP_SYSFS_SUPPORT)
	struct class_device *nd_class_dev;	/* Hang our sysfs stuff off of here */
	struct class_device *nd_class_ports_dev;
#endif
};


/************************************************************************
 * Defines to keep track of time and to manage the poller.
 ************************************************************************/
#define TimeGE(a, b) ((long) ((a) - (b)) >= 0)
#define dgrp_jiffies_from_ms(a) (((a) * HZ) / 1000)

extern ulong drp_poll_time;		/* Time of next poll */
