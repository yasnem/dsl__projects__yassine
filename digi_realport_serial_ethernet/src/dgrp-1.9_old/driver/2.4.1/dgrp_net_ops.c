 ...
 *  Filename:
 *
 *     $Id: dgrp_net_ops.c,v 1.2 2010/02/04 20:05:58 scottk Exp $
 *
 *  Description:
 *
 ...
 */
STATIC int             FN(net_open)   (struct inode *, struct file *);
STATIC CLOSE_RETURN_T  FN(net_release)(struct inode *, struct file *);
STATIC READ_RETURN_T   FN(net_read)   (READ_PROTO);
STATIC WRITE_RETURN_T  FN(net_write)  (WRITE_PROTO);
STATIC int             FN(net_ioctl)  (struct inode *, struct file *,
                                       unsigned int, unsigned long);
STATIC SELECT_RETURN_T FN(net_select) (SELECT_PROTO);

/*
 *  Inode operation declarations
 */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,0)
static int net_chk_perm(struct inode *, int, struct nameidata *);
#else
...
static struct file_operations net_ops =
{
	...
	owner:	THIS_MODULE,
#endif
	read: FN(net_read),	/* read	   */
	write: FN(net_write),	/* write   */
	SELECT_NAME: FN(net_select),	/* poll or select */
	ioctl: FN(net_ioctl),	/* ioctl   */
	open: FN(net_open),	/* open    */
	release: FN(net_release),/* release */
};

STATIC struct inode_operations net_inode_ops =
{
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,3,0)
	default_file_ops: &net_ops,
#endif
...
int register_net_device(struct nd_struct *node, struct proc_dir_entry *root)
{
	...
	PROC_DE_OPS_INIT(de, &net_inode_ops, &net_ops);
	node->nd_net_de = de;
	init_MUTEX(&node->nd_net_semaphore);
	init_MUTEX(&node->nd_writebuf_semaphore);
...
static void dgrp_input(struct ch_struct *ch)
{
	...
	tty_count = DGRP_GET_TTY_COUNT(tty);
	if (!tty_count) {
		ch->ch_rout = ch->ch_rin;
		DGRP_UNLOCK(nd->nd_lock, lock_flags);
	...
	if (GLBL(rawreadok) && tty->real_raw)
		flip_len = MYFLIPLEN;
	else
	...
	dbg_net_trace(INPUT, ("rawreadok(%d) real_raw(%d) flip_len(%d) data_len(%d) len(%d)\n", 
		GLBL(rawreadok), tty->real_raw, flip_len, data_len, len));
	...
	if (len && !(ch->ch_flag & CH_RXSTOP)) {
		...
		if (GLBL(rawreadok) && tty->real_raw) 
			dbg_net_trace(INPUT, ("We have rawreadok and tty->real_raw!\n"));

		dbg_net_trace(INPUT, ("len(%d)\n", len));

		if (nd->nd_dpa_debug && nd->nd_dpa_port == PORT_NUM(DGRP_TTY_MINOR(tty))) {
		 ...
		 */
		if (GLBL(rawreadok) && tty->real_raw) {
			ld->receive_buf(tty, myflipbuf, NULL, len);
		} else {
...
static void dgrp_input(struct ch_struct *ch)
{
	...
	int	tty_count;
	ulong	lock_flags;
	static char  myflipbuf[MYFLIPLEN];
	struct tty_ldisc *ld;
	...
	dbg_net_trace(KME, ("-KME- flip.char_buf(%x), char_buf_ptr(%x) count(%d)\n", 
		tty->flip.char_buf, tty->flip.char_buf_ptr, tty->flip.count));
	dbg_net_trace(KME, ("flip.flag_buf(%x), flag_buf_ptr(%x) buf_num(%d)\n", 
		tty->flip.flag_buf, tty->flip.flag_buf_ptr, tty->flip.buf_num));
	
	/*
	 *      If the rxbuf is empty and we are not throttled, put as much
	 *      as we can directly into the linux TTY flip buffer.
	 *      The rawreadok case takes advantage of carnal knowldge that
	...
	if (GLBL(rawreadok)) {
		...
		flip_len = TTY_FLIPBUF_SIZE - tty->flip.count;
	}
	...
	if (len && !(ch->ch_flag & CH_RXSTOP)) {
		...
		if (GLBL(rawreadok)) {
			if(tty->real_raw)
				...
				dgrp_read_data_block(ch, tty->flip.char_buf, len, len);
		} else {
			dgrp_read_data_block(ch, tty->flip.char_buf_ptr, len, len);
		}
		...
		if(!GLBL(rawreadok)) {
			/*
			 * we used the tty->flipbuf, now update the ptrs 
			 * and the flag_buf
			 */
			if (I_PARMRK(tty)) { 
				parity_scan(ch, tty->flip.char_buf_ptr,
		...
		} else if (!tty->real_raw) {
			if (I_PARMRK(tty)) { 
				parity_scan(ch, tty->flip.char_buf,
				    tty->flip.flag_buf, &len) ;
			} else {
				memset(tty->flip.flag_buf, TTY_NORMAL, len);
			}
		}
		...
		if (GLBL(rawreadok)) {
			if(tty->real_raw) {
				...
				if (nd->nd_dpa_debug && nd->nd_dpa_port == PORT_NUM(DGRP_TTY_MINOR(tty))) {
					dgrp_dpa_data(nd, 1, tty->flip.char_buf, len);
				}
				ld->receive_buf(tty, tty->flip.char_buf,
				    tty->flip.flag_buf, len);
			}
		} else {
...
*
*  TODO:  do we need to do the XMODEM, XOFF, XON, XANY processing?? as per protocol
*
******************************************************************************/
 ...
 */
			...
			tbuf = (uchar *) kmalloc(TBUF_MAX, GFP_KERNEL);
			rbuf = (uchar *) kmalloc(RBUF_MAX, GFP_KERNEL);
 ...
 */
			...
			ch->ch_tbuf = tbuf;
			ch->ch_rbuf = rbuf;
#if defined(DGRP_SYSFS_SUPPORT) && defined(REGISTER_TTYS_WITH_SYSFS)
			{
				...
				ret = sysfs_create_link(&nd->nd_class_ports_dev->kobj,
					&classp->kobj, name);
...
#elif defined(REGISTER_TTYS_WITH_SYSFS)
			tty_register_device(&nd->nd_serial_ttdriver, i, NULL); 
			if (GLBL(register_cudevices))
				tty_register_device(&nd->nd_callout_ttdriver, i, NULL);
			if (GLBL(register_prdevices))
				tty_register_device(&nd->nd_xprint_ttdriver, i, NULL);
#endif
			nd->nd_chan_count = ++i;
 ...
 */
...

			{
				...
				snprintf(name, DEVICE_NAME_SIZE, "tty_%d", i);
				sysfs_remove_link(&nd->nd_class_ports_dev->kobj, name);
...
static void dgrp_encode_time(struct nd_struct *nd, uchar *buf)
{
	...
	FN(encode_u4)(buf, (uint)(t & 0xffffffff));
}
...
STATIC int FN(net_open) (struct inode *inode, struct file *file)
{
	struct nd_struct *nd;
	int     rtn = 0;
	ulong  lock_flags;
	...
	nd->nd_iobuf = (uchar *) kmalloc(UIO_MAX + 10, GFP_KERNEL);
	if (!nd->nd_iobuf) {
		rtn = -ENOMEM;
		goto unlock;
	}
	 ...
	 */
	nd->nd_writebuf = (uchar *) kmalloc(WRITEBUFLEN, GFP_KERNEL);
	if (!nd->nd_writebuf) {
		rtn = -ENOMEM;
		goto unlock;
	}
...
STATIC CLOSE_RETURN_T FN(net_release) (struct inode *inode, struct file *file)
{
	struct nd_struct *nd;
	uchar *iobuf;
	ulong  lock_flags;
...
static int dgrp_send(struct nd_struct* nd, long tmax)
{
	...
	if (nd->nd_send & NR_PASSWORD) {
		...
		b[0] = 0xfc;
		b[1] = 0x20;
		FN(encode_u2)(b + 2, strlen(nd->password));
		b += 4;
	...
	for (mod = 0; port < nd->nd_chan_count; mod++) {
		...
		for (; port < maxport; port++, ch++) {
			...
			case CS_IDLE:
				{
					/*
					 *  Wait until any open error code has been
					 *  delivered to all associated ports.
					 */
			...
			case CS_READY:
				...
				if ((ch->ch_flag & (CH_RX_FLUSH | CH_PARAM)) != 0) {
					...
					if ((ch->ch_flag & CH_RX_FLUSH) != 0) {
						if (((ch->ch_flush_seq - nd->nd_seq_out) & SEQ_MASK ) >
							...
							dbg_net_trace(READ, ("dgrp send(%x:%x) RX has flushed\n",
								DGRP_TTY_MAJOR(ch->ch_tun.un_tty),
					...
					if (ch->ch_s_tmax  != ch->ch_tmax ||
						...
						b[0] = 0xb0 + (port & 0xf);
						b[1] = 48;
						FN(encode_u2)(b + 2, ch->ch_s_tmax  = ch->ch_tmax);
						FN(encode_u2)(b + 4, ch->ch_s_ttime = ch->ch_ttime);
						b += 6;
					}
					...
					if (ch->ch_s_rlow  != ch->ch_rlow ||
						...
						b[0] = 0xb0 + (port & 0xf);
						b[1] = 45;
						FN(encode_u2)(b + 2, ch->ch_s_rlow  = ch->ch_rlow);
						FN(encode_u2)(b + 4, ch->ch_s_rhigh = ch->ch_rhigh);
						b += 6;
					}
					...
					if (ch->ch_s_brate != ch->ch_brate ||
						...
						FN(encode_u2)(b +  2, ch->ch_s_brate = ch->ch_brate);
						FN(encode_u2)(b +  4, ch->ch_s_cflag = ch->ch_cflag);
						FN(encode_u2)(b +  6, ch->ch_s_iflag = ch->ch_iflag);
						FN(encode_u2)(b +  8, ch->ch_s_oflag = ch->ch_oflag);
						FN(encode_u2)(b + 10, ch->ch_s_xflag = ch->ch_xflag);
						b += 12;
					}
					...
					if (ch->ch_s_rmax != ch->ch_rmax ||
						...
						b[0] = 0xb0 + (port & 0xf);
						b[1] = 47;
						FN(encode_u2)(b + 2, ch->ch_s_rmax  = ch->ch_rmax);
						FN(encode_u2)(b + 4, ch->ch_s_rtime = ch->ch_rtime);
						b += 6;
					}
				...
				if (ch->ch_send != 0) {
					...
					if ((send & RR_TX_BREAK) != 0) {
						if (ch->ch_break_time != 0) {
							b[0] = 0xb0 + (port & 0xf);
							b[1] = 61;
							FN(encode_u2)(b + 2, ch->ch_break_time);
							b += 4;
					...
					if ((send & (RR_RX_FLUSH | RR_TX_FLUSH)) != 0) {
						...
						dbg_net_trace(OUTPUT, ("dgrp send(%x:%x) sending FLUSH command %d\n",
							DGRP_TTY_MAJOR(ch->ch_tun.un_tty),
							DGRP_TTY_MINOR(ch->ch_tun.un_tty), b[2]));
						b += 3;
				...
				if (n >= RBUF_MAX / 4) {
					b[0] = 0xa0 + (port & 0xf);
					FN(encode_u2)(b + 1, ch->ch_s_rwin = rwin);
					b += 3;
				}
				...
				if (ch->ch_pun.un_open_count &&
					...
					if ((ch->ch_pun.un_flag & UN_LOW) != 0 ?
					    ...
					    ( (ch->ch_tun.un_open_count &&
					      ch->ch_tun.un_tty->ldisc.chars_in_buffer) ?
	 ...
	 */
		...
		if (tsend < 2) {
#if 0
			printk("tsend: %ld tm: %ld count: %d tchan: %ld\n",
				tsend, tm, nd->nd_chan_count, tchan);
#endif
			tsend = 1;
		}
	}
	...
	for (mod = 0; port < nd->nd_chan_count; mod++) {
		...
		for (; port < maxport; port++, ch++) {
			...
			if ( n != 0 && used_buffer > 0) {
				...
				if (n > used_buffer) {
#if 0
					printk("Data Push: n: %ld tsend: %ld used: %ld cur: %ld\n", n, tsend, used_buffer, port);
#endif
					work = 1;
					n = used_buffer;
				}
			...
			if ((ch->ch_flag & (CH_EMPTY | CH_DRAIN)) != 0 ||
				...
				if (ch->ch_s_treq != ch->ch_s_tin) {
					b[0] = 0xb0 + (port & 0xf);
					b[1] = 43;
					FN(encode_u2)(b + 2, ch->ch_s_treq = ch->ch_s_tin);
					b += 4;
				}
	...
	if (n > tsafe) {
#if 1
		printk("n: %ld tsafe: %ld tsend: %ld ttotal: %ld tmax: %ld tchan: %ld\n",
 ...
 */
	...
	if (b > nd->nd_iobuf + UIO_MAX) {
#if 1
		printk("b: %p nd_iobuf + UIO_MAX: %p\n", b, nd->nd_iobuf + UIO_MAX);
#endif
	}
	...
	while (remain > 0) {
		...
		data:
			...
			dbg_net_trace(INPUT, ("in dgrp_receive(%x:%x) state(%x) cflag(%x) flag(%x)\n",
				DGRP_TTY_MAJOR(ch->ch_tun.un_tty), DGRP_TTY_MINOR(ch->ch_tun.un_tty),
		...
		case 11:
			...
			case 15:
				 ...
				 * drp.h for ch_s_rin variable.
					int rin = FN(decode_u2)(b + 2);
					int tpos = FN(decode_u2)(b + 4);
				*/
			...
			case 17:
				...
				{
					ch->ch_s_elast = FN(decode_u2)(b + 2);
					ch->ch_s_mlast = b[4];
			...
			case 21:
				...
				{
					ch->ch_s_rsize = FN(decode_u2)(b + 2);
					ch->ch_s_tsize = FN(decode_u2)(b + 4);
		...
		case 12:
			...
			estat = ch->ch_s_elast = FN(decode_u2)(b + 2);
 ...
 */
	...
	case SEL_IN:
		...
		dbg_net_trace(SELECT, ("net select(%p) waiting to read\n", nd));

		select_wait(&nd->nd_tx_waitq, table);
		
		break;

	case SEL_OUT:
		retval = 1;                    /* Always writeable */
		break;
	}
	

#else   /* __USE_OLD_SELECT__ */
...
STATIC int FN(net_ioctl) (struct inode *inode, struct file *file,
...
******************************************************************************/
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,0)
static int net_chk_perm(struct inode *inode, int op, struct nameidata *notused)
#else
