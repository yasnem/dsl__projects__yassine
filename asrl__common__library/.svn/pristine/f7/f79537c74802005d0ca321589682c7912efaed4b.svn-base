...
namespace asrl{
    ...
    m_maxTagLength(0)
  {
    ...
    ASRL_ASSERT_NE(returnCode, 0,"Unable to query the performance frequency");
  ...
  std::string Timing::getTag(size_t handle){
    ...
    ASRL_ASSERT(TimerError,found,"Unable to find the tag associated with handle " << handle);
    return tag;
  }
  ...
  {
    ASRL_ASSERT_LT(TimerError,handle, Timing::instance().m_timers.size(),"The handle is invalid. Handle: " << handle << ", number of timers: " << Timing::instance().m_timers.size());
    if(!constructStopped)
      start();
  }
  ...
  void Timer::start(){
    ASRL_ASSERT(TimerError,!m_timing,"The timer " + Timing::getTag(m_handle) + " is already running");
    m_timing = true;
#ifdef ASRL_USE_HIGH_PERF_TIMER
  ...
  void Timer::stop()
  {
    ASRL_ASSERT(TimerError, m_timing,"The timer " + Timing::getTag(m_handle) + " is not running");
    double dt;
#ifdef ASRL_USE_HIGH_PERF_TIMER
    LARGE_INTEGER end;
  ...
  double Timing::getTotalSeconds(size_t handle) {
    ASRL_ASSERT_LT(TimerError, handle, instance().m_timers.size(), "Handle is out of range: " << handle << ", number of timers: " << instance().m_timers.size());
