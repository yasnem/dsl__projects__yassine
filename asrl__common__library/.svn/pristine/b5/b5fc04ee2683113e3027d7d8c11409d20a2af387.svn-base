...
namespace asrl { namespace io 
{
  ...
  {
    validateName(scalarName,ASRL_SOURCE_FILE_POS);
  ...
  void MatrixArchive::getMatrix(std::string const & matrixName, Eigen::MatrixXd & outMatrix) const
  {
    ...
    if(it == m_values.end())
      {
	ASRL_THROW(MatrixArchiveException,"There is no matrix named \"" << matrixName << "\" in the archive");
      }
  ...
  void MatrixArchive::getVector(std::string const & vectorName, Eigen::VectorXd & outVector) const
  {
    ...
    if(it == m_values.end())
      {
	ASRL_THROW(MatrixArchiveException, "There is no vector named \"" << vectorName << "\" in the archive");
      }
    Eigen::MatrixXd const & M = it->second;
    ASRL_ASSERT_EQ(MatrixArchiveException, M.cols(), 1, "The stored value is not a vector");
  ...
  void MatrixArchive::getScalar(std::string const & scalarName, double & outScalar) const
  {
    ...
    if(it == m_values.end())
      {
	ASRL_THROW(MatrixArchiveException, "There is no scalar named \"" << scalarName << "\" in the archive");
      }

    Eigen::MatrixXd const & M = it->second;
    ASRL_ASSERT_EQ(MatrixArchiveException, M.rows(), 1, "The stored value is not a scalar");
    ASRL_ASSERT_EQ(MatrixArchiveException, M.cols(), 1, "The stored value is not a scalar");

    outScalar = M(0,0);
  }
  
  void MatrixArchive::validateName(std::string const & name, asrl::source_file_pos const & sfp) const
  {
    if(name.size() > s_fixedNameSize || name.size() == 0)
      {
	ASRL_THROW_SFP(MatrixArchiveException, sfp, "The name \"" << name << "\" is an incorrect size. Names length must be between 1 and " << s_fixedNameSize);
      }

    ASRL_ASSERT(MatrixArchiveException, isalpha(name[0]), "The name \"" << name << "\" is invalid. The first character of the name must be a letter");
    
    for(unsigned i = 1; i < name.size(); i++)
      {
	ASRL_ASSERT(MatrixArchiveException, isalnum(name[i]) || name[i] == '_', "The name \"" << name << "\" is invalid. The characters of the name must be alphanumeric or an underscore. (failed at character " << i << ")");
      }
  }
  ...
  void MatrixArchive::writeMatrix(std::ostream & fout, std::string const & name, Eigen::MatrixXd const & matrix) const
  {
    ...
    // 64 character name
    validateName(name, ASRL_SOURCE_FILE_POS);
    fout.fill(' ');
  ...
  {
    ASRL_THROW(MatrixArchiveException, "Not Implemented");
  }
  ...
  void MatrixArchive::readMatrix(std::istream & fin, std::string & name, Eigen::MatrixXd & matrix) const
  {
    ...
    // start character
    fin.read(&start,1);
    ASRL_ASSERT_EQ(MatrixArchiveException, start, s_magicCharStart, "The matrix block didn't start with the expected character");
    ...
    // end character
    fin.read(&end,1);
    ASRL_ASSERT_EQ(MatrixArchiveException, end, s_magicCharEnd, "The matrix block didn't end with the expected character");
  }
  ...
  void MatrixArchive::load(std::string const & asaFilePath)
  {
    std::set<std::string> names;
    load(asaFilePath,names);
  }
  void MatrixArchive::load(boost::filesystem::path const & asaFilePath)
  {
    std::set<std::string> names;
    load(asaFilePath,names);
  }
  ...
  void MatrixArchive::save(std::string const & asaFilePath) const
  {
    std::set<std::string> names;
    save(asaFilePath, names);
  }


  void MatrixArchive::save(boost::filesystem::path const & asaFilePath) const
  {
    std::set<std::string> names;
    save(asaFilePath, names);
  }
  ...
  void MatrixArchive::append(std::string const & asaFilePath) const
  {
    std::set<std::string> names;
    append(asaFilePath,names);
  }

  void MatrixArchive::append(boost::filesystem::path const & asaFilePath) const
  {
    std::set<std::string> names;
    append(asaFilePath,names);
  }

  void MatrixArchive::save(boost::filesystem::path const & asaFilePath, std::set<std::string> const & validNames) const
  {
    std::ofstream fout(asaFilePath.string().c_str(), std::ios::binary);
    ASRL_ASSERT(MatrixArchiveException, fout.good(), "Unable to open file " << asaFilePath.string() << " for writing");
	...
	if(validNames.empty() || validNames.count(it->first) > 0)
	  {
	    ...
	    ASRL_ASSERT(MatrixArchiveException, fout.good(), "Error while writing matrix " << it->first << " to file.");
	  }
      }
  }

  void MatrixArchive::load(boost::filesystem::path const & asaFilePath, std::set<std::string> const & validNames)
  {
    std::ifstream fin(asaFilePath.string().c_str(), std::ios::binary);
    ASRL_ASSERT(MatrixArchiveException, fin.good(), "Unable to open file " << asaFilePath << " for reading");
  ...
  void MatrixArchive::append(boost::filesystem::path const & asaFilePath, std::set<std::string> const & validNames) const
  {
    std::ofstream fout(asaFilePath.string().c_str(), std::ios::binary | std::ios::app);
    ASRL_ASSERT(MatrixArchiveException, fout.good(), "Unable to open file " << asaFilePath.string() << " for writing");
  }
