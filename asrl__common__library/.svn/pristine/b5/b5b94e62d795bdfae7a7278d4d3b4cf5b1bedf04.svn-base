...
namespace asrl { namespace math {
    ...
    Eigen::Matrix<double,6,6> toTEulerInvSMatrix(const Eigen::Matrix<double,6,1> & t)
    {
      Eigen::Matrix<double,6,6> invS = Eigen::Matrix<double,6,6>::Identity();
      
      invS.bottomRightCorner<3,3>() = asrl::math::rph_S(-t.tail<3>()).inverse();
      invS.topRightCorner<3,3>() = asrl::math::crossMx(t.head<3>());
