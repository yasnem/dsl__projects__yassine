...
namespace asrl { namespace math {
    ...
    Eigen::Matrix4d threePointSvd(Eigen::MatrixXd const & p0, Eigen::MatrixXd const & p1)
    {
      ...
      ASRL_ASSERT_EQ_DBG(std::runtime_error, p0.rows(), 3, "p0 must be a 3xK matrix");
      ASRL_ASSERT_EQ_DBG(std::runtime_error, p1.rows(), 3, "p1 must be a 3xK matrix");
      ASRL_ASSERT_EQ_DBG(std::runtime_error, p0.cols(), p1.cols(), "p0 and p1 must have the same number of columns");
    ...
    Eigen::Matrix3d qMethod(Eigen::MatrixXd const & p0, Eigen::MatrixXd const & p1, const Eigen::VectorXd & w)
    {
      ...
      ASRL_ASSERT_EQ_DBG(std::runtime_error, p1.rows(), 3, "p1 must be a 3xK matrix");
      ASRL_ASSERT_EQ_DBG(std::runtime_error, p0.cols(), p1.cols(), "p0 and p1 must have the same number of columns");
      ASRL_ASSERT_EQ_DBG(std::runtime_error, w.size(), p0.cols(),  "w must have the same number of columns as p0");
	...
	{
	  double wi = sqrt(w[i]);
	  ASRL_ASSERT_EQ_TOL_DBG(std::runtime_error, p0.col(i).norm(),1.0,1e-4,"Column " << i << " of p0 was not a unit vector");
	  ASRL_ASSERT_EQ_TOL_DBG(std::runtime_error, p1.col(i).norm(),1.0,1e-4,"Column " << i << " of p1 was not a unit vector");
      ...
      for(int i = 1; i < 4; i++)
	{
      ...
      Eigen::Matrix3d C_01 = (qs*qs - qv.dot(qv))*Eigen::Matrix3d::Identity() + 2.0 * qv * qv.transpose() - 2.0 * qs * asrl::math::crossMx(qv);
