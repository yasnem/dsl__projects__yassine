...
namespace asrl { namespace math {
     ...
     */
    Eigen::Matrix<double,6,1> fromTEuler(const Eigen::Matrix4d & T_ab); //Be careful, this gives you back T_ab(1:3,4), i.e., the xyz points of a in frame b -- JDG
     ...
     * NOTE: I BELIEVE THIS IS BOX-DOT AS OF 2012-07-XX
     *
    ...
    // The matrix that, to first order, relates a small transformation matrix perturbation
    // dt to a small perturbation in "Euler" representation so that:
    // de = invS * dt
    Eigen::Matrix<double,6,6> toTEulerInvSMatrix(const Eigen::Matrix<double,6,1> & dt);
