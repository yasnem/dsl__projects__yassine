#ifndef ASRL_ASA_MATRIX_IO_HPP
#define ASRL_ASA_MATRIX_IO_HPP
...
namespace asrl { namespace io {
    ...
    ASRL_DEFINE_EXCEPTION(MatrixArchiveException,std::runtime_error);
    ...
    public:
      ...
      void load(const std::string & asaFilePath);
      void load(boost::filesystem::path const & asaFilePath);
      void load(boost::filesystem::path const & asaFilePath, std::set<std::string> const & validNames);

      // Saves matrices from a file into the archive.
      void save(const std::string & asaFilePath) const;
      void save(boost::filesystem::path const & asaFilePath) const;
      void save(boost::filesystem::path const & asaFilePath, std::set<std::string> const & validNames) const;
      ...
      void append(boost::filesystem::path const & asaFilePath) const;
      void append(boost::filesystem::path const & asaFilePath, std::set<std::string> const & validNames) const;
    ...
    private:
      ...
      void validateName(std::string const & name, asrl::source_file_pos const & sfp) const;
    ...
    {
      validateName(matrixName, ASRL_SOURCE_FILE_POS);
    ...
    {
      ASRL_ASSERT_EQ(MatrixArchiveException, vector.cols(),1, "The input must be a column vector");
      validateName(vectorName, ASRL_SOURCE_FILE_POS);
      m_values[vectorName] = vector;
    }


  }} // end namespace asrl::io
