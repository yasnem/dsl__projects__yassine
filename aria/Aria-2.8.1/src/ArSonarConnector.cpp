  ...
  bool powerCycleSonarOnFailedConnect)
{
  ...
  ArLog::log(ArLog::Normal, "ArSonarConnector::connectSonars(), finally connecting to each sonar...");
	for (it = mySonars.begin(); it != mySonars.end(); it++) {
		sonarData = (*it).second;
...
//	     (sonarData->myPortType != NULL && strlen (sonarData->myPortType) > 0)) {
		...
		ArLog::log (ArLog::Normal, "ArSonarConnector::connectSonars() sonar #%d type= %s port=%s portType=%s baud=%s autoconnect=%s, connect=%d",
		            sonarData->myNumber,
		            sonarData->myType,
		            sonarData->myPort,
		            sonarData->myPortType,
