...
namespace asrl {
  namespace vehicle {
    ...
    void Base::initializeMessages(void) {
      ...
      odometryTransformMessage_.header.frame_id =  vehicleName_ + "_odom";
      odometryTransformMessage_.child_frame_id = vehicleBaseFrameName_;
    ...
    void Base::publishOdometry(const ros::TimerEvent& event)
    {
      ...
      if ( fillOdometryMessage() ) {
        ...
        tf::quaternionTFToMsg(btTrans.getRotation(), quat_msg);

        odometryTransformMessage_.header.stamp = odometryMessage_.header.stamp;
        odometryTransformMessage_.transform.translation.x = btTrans.getOrigin().x();
        odometryTransformMessage_.transform.translation.y = btTrans.getOrigin().y();
        odometryTransformMessage_.transform.translation.z = btTrans.getOrigin().z();
